...
namespace NSI.BusinessLogic.Implementations
{
    public class PdfManipulation : IPdfManipulation
    {
        ...
        public byte[] CreatePassportPdf(Document document, User user)
        {
            return CreatePdf(document, user);
        }

        public byte[] CreateVisaPdf(Document document, User user)
        {
            return CreatePdf(document, user);
        }

        private byte[] CreatePdf(Document document, User user)
        {
            ...
            {
                PagesCount = true,
                HtmlContent = document.Type.Name.Equals("Passport") ? TemplateGenerator.GetPassportHtmlString(document, user) : TemplateGenerator.GetVisaHtmlString(document, user),
